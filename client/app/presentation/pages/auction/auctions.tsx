  ...
  return (
    <main className="max-w-7xl mx-auto p-6 space-y-10">
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        ...
        >
          {isTesting ? "Injecting Load into the Network..." : "Simulate Load Test"}
        </button>
      </header>
      {notification && (
        <div className={`p-4 rounded-lg font-mono text-sm shadow-sm border ${notification.type === 'success'
          ...
          {notification.type === 'success' && <span className="block mt-2 opacity-75 animate-pulse">Forcing topological synchronization in 3 seconds...</span>}
        </div>
      )}
      ...
      <section className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm p-6 relative">
        ...
        {(isCreating || isTesting) && (
          <div className="absolute inset-0 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm z-10 flex items-center justify-center rounded-2xl">
            <p className="font-mono text-indigo-600 dark:text-indigo-400 animate-pulse font-bold">Processing transaction on the network...A processar transação na rede...</p>
          </div>
        )}
